...
void worker_thread(session_t *session);
...
int main(int argc, char **argv) {
  ...
  // Initialize pipe
  int register_fifo;
  if (initialize_pipe(&register_fifo, argv[4])) {
    return 1;
  }
  ...
  while(1){
    ...
    if (num_read > 0){
      if(buf[0]=='1'){
        ...
        printf("%s\n",noti_pipe_path);
        //worker_thread(req_pipe_path,resp_pipe_path,noti_pipe_path);
        create_session(&session, req_pipe_path, resp_pipe_path, noti_pipe_path,1);

        worker_thread(&session);
      }
      break;
    }
  }
  //cada vez que há um connect cria uma nova thread gestora que vai cuidar desse connect
